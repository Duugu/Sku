...
local function drawMinimapPin(pin, data)
    ...
    if dist > 1 and data.floatOnEdge and data.skuOverdraw ~= true then
        dist = dist^0.5
        diffX = diffX/dist
        diffY = diffY/dist
    end

    if dist <= 1 or data.floatOnEdge or data.skuOverdraw == true then
        pin:Show()
        pin:ClearAllPoints()
...
function pins:AddMinimapIconWorld(ref, icon, instanceID, x, y, floatOnEdge, skuOverdraw)
    if not ref then
