...
- Added a new experimental feature to monitor the health of party members: Core > Monitor > Party > Health</br>
	...
	The idea is to have a more constant, less cluttered/annoying, and more intuitive output than the one, one, one, etc. from Audio QS.
	I would assume that, if one gets used to the output style, the output speed could be much faster (if you would like to try, lower the "Speed" option to 20, the default is 50).</br>
