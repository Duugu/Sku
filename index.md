...
- [SkuMapper r4.6](https://github.com/Duugu/SkuMapper/releases/download/r4.6/SkuMapper-r4.6-cata.zip) (January 27th 2025), mapping addon for sighted players<br>
...
## Changes in Wow Login Tool r1.12
...
- A lot of meaningless objects are removed. Blizzard has added a load of senseless objects with Cata. Chairs, campfires, lamps, and a lot more. I've removed a lot of those objects. But there are still loads of objects with no use. Like sign posts. I can't remove them. So, a waypoint like "object 50501 hunter rise thunder bluff 2" most likely doesn't mean this is on hunter rise in thunder bluff. It just is a sign post somewhere, showing the direction to the hunter rise.
- All default waypoints to creatures and objects are added to the shift + f10 Destinations by Distance list. So, this list now has also routes to waypoints that are not directly linked by mappers, and where you had to use shift + f9 > ... > Close route before. These waypoints are prefix with "Close".
- The newly added "Close" routes in the shift + f10 menu are depending on a pre-calculated table of close linked waypoints. This pre-calculated table is build by Sku on the very first time you're logging in and will be saved. The calculation is done in the background and will degrade the frame rate. It shouldn't have much noticable effects (at least if your computer isn't from the last century). It may take up to 2 hours or even more. If it is completed, the pre-calculation won't happen again until a Sku update is adding new map data. If you're logging out or reloading before the pre-calculation is completed, it will be continued on next login / after reload. The shift + f10 menu will list only close routes with already pre-calculation completed for them. Therefore, you won't find a lot of them directly after first login, and their number will grow and grow, until the full pre-calculation is completed.
- The Navigation > Waypoint > Select > Current Map by Distance > ... > Close route (shift + f9) menu now has an entry point selection like the routes menu (shift + f10), instead of just silently taking the closest linked waypoint an an entry point. Additionally the menu is listing up to 10 close waypoints to select from, instead of just taking the closest one.
- A lot of map issues are fixed. Like with Strangle, Barrens, etc.
- Adapted the action button menu to the new flyouts.
...
- Tried to fix a bug with incoming heal in monitors.
...
- Added a new option: Chat > Options > Do not read out emojis. Default: off

## Changes in Sku r35.8
- Fixed a bug with the new routes algorithm that could lead to unreachable waypoints
- Removed routes to auto waypoints from routes list (shift + f10)
- Enhanced the performance of the routes menu (shift + f10) by limiting the listed routes to a maximum of 5000 meters distance
- New option: Navigation > Options > Maximum distance for destinations in routes list. Default: 5000 meters. Use a lower value to enhance the performance of the routes menu (shift + f10.)
...
- Added a new attribute for aura conditions: Spell usable (L)<br>
	...
	There are some important things to consider with this specific aura attribute:<br>
...
<br>Caution: This is a complex feature that needs to be tested carefully. I did tests, but errors are of course possible. In worst case you could lose all your existing auras.
<br>I highly suggest to export and save ALL your existing auras before taking hands on it!
<br>
<br>You now can use other sku auras as regular conditions in auras. So, you can "nest" auras and build much, much more complex and flexible logic in auras.
<br>On building a new aura the appropiate (more below) existing auras are available in the attributes list with their aura name (and the prefix "sku aura").
<br>If you're adding an existing aura as a condition in a new aura, the addon will evaluate the full existing aura like every other condition in that aura. 
<br>That is, if the result of the existing aura is or is not true or false.
<br>
<br>Until now there wasn no control over how aura conditions were evaluated.
<br>All conditions in an aura were evaluated using AND in there order of occurence. Like "if condition 1 AND condition 2 AND condition 3".
<br>That was restricting use cases and made some scenarios impossible.
<br>Example: You would like to build an aura that is notifying you if you have five stacks of some buff named "Explosion" AND if your target is casting Fireball.
<br>In pseudo aura code that would be something like "if source equal self and aura stacks equal 5 and and spell name equal Explosion and source equal target and event equal spell cast <br>start and spell name equal Fireball".
<br>I guess it is clear why that wasn't possible, as you would have to use two "spell name equal" conditions in one aura, etc.
<br>From now on you can build a first aura to test if your target is casting Fireball, and a second aura to test if you have 5 stacks of Explosion AND if the first aura is true.
<br>
<br>Example:
<br>First aura, with the custom name "casting fireball": if source (L) contains target AND event equal spell cast start AND spell name equal fireball.
<br>Second aura: if source (L) contains self AND aura stack equal 5 AND spell name equal Explosion AND sku aura casting fireball equal true then... some output
<br>The first aura "casting fireball" has no own output. It is just there to be used as an additional condition in the second aura.
<br>
<br>Requirements:
- You can only use auras with custom names in other auras. Auras with auto generated names from their conditions can't be used. So, if you would like to use an aura in other auras, you need to explicit rename that aura.
- You can't delete any auras that are currently used in other auras. Remove them from all other auras first.
- You can always rename auras. But you can't set auras back to "auto generate name from conditions" as long as they are used in other auras.
- You can't nest auras for more than one level. If you're using aura A in aura B, then you can't use aura B in aura C.
- But you can of course use an aura in more than one other auras. A in B and A in C and A in D is ok.
- There is a new action type "no action" and a new output "nothing". Use them to avoid that any auras you would nest into other auras later on are triggering unwanted outputs. Example: "then no action nothing".
...
- Added a new feature: Turn to Unit <br>
	...
	- Speed: the faster, the less accurate
	- Enhanced settings: do not touch unless prompted
- Fixed a bug with the Human tutorials step "start navigation to class trainer"
- Changed "heavy armor" to "chain armor" in tutorial texts
- Fixed an issue with missing success sound in tutorials if you are not on the latest SkuAudiodata version
- It is now possible to use the "next step" key bind (default: ALT + J) to jump to the next tutorial step even if you haven't completed the current step. You will hear a warning and need to confirm by pressing ALT + J again to actually jump to the next step. This most likely will break the tutorial and is not recommended.
- Fixed the missing "dead" output on targeting dead targets
- Added a Core > Dungeon and raid settings menu to set the difficulty if you are the party/raid leader
- Fixed an issue with missing audio outputs for female creatures in German
- Fixed an issue on Mac clients where tts outputs could not be canceled anymore. This will break the tutorials feature on Mac clients, but I can't do anything to avoid that. It's Blizzards fault.
- Fixed an issue with "auto" waypoints incorrectly showing up in the waypoint list (shift + f9)
- The addon is now outputting generic emojis like in chat messages as text that is correctly read by tts voices.
...
- Fixed a bug with missing sound for units that are already in combat. Should now be the beep sound as default.
- Added a filter to automatically remove the annoying "Citizens of Dalaran..." outputs that Rhonin is doing if someone has killed Algalon. Removes the chat and audio output.
- Added a new chat feature: Chat > Filter. You can add and remove filter terms or phrases to a list. If a chat message contains any filter term, it will be ignored for all chat tabs. Keep that in mind. If you are adding very generic terms, then a lot of messages could be filterd. :) Filter terms are not case-sensitive. Filter terms are stored per Sku profile.
- Added a new feature: private Sku target markers
	Sku target markers are similar to the games target markers (Skull, Cross, Moon, etc.), but they are using colors instead of symbold and they private to you. They are not visible for other players. The colors are: White, Red, Blue, Green, Purple, Yellow, Orange<br>
	The Sku target markers are to uniquely mark and identify units. Example, if you are fighting three Young Boars, you can privately mark the first one with White, the second with Red, and the third with Blue. The sku target markers are added before the mob name. Similar to the standard raid target markers. So, with the second Young Boar marked with "Blue", you will hear "Blue Young Boar Level..." on targeting that boar.
	That way you can uniquely identify all mobs, even if they have the same name.<br>
	As stated, the Sku markers are private and additionally to the games standard target markers. So, even in a group they won't mix up with the standard target markers, an won't confuse other players.<br>
	There are two ways to set sku target markers:
	- There are 8 new key binds for them: Core > Sku key binds > bind key > Set Sku target marker x. (Just filter for "sku target marker")
	- There is a new option to auto-assign them: Mob > Options > Auto set private Sku raid targets on in combat targets without a raid target. Default: Off. With this set to On, the addon will auto assign a Sku marker to every unit that is in combat with you and don't have and marker (Sku or Game) yet. Markers will be removed if a unit dies. 
	- There is a sku key bind to additionally clear all assigned Sku markers at once.
	
	Some more details:
	- A unit can't have a sku marker and a game marker. Therefore the games target markers are overriding the sku target markers. If you have privately marked a mob as blue, and you or someone else is marking that mob as the default target marker "Skull", then Blue will just be replaced by Skull.
	- The default games Remove Target Marker feature is used to remove a sku marker. That is under Core > Game key binds > Assign key > Target markers > Remove target marker. The default binding is Control + numpad 0.
	- Good to know: for easier referencing the sku target markers are using the same colors as the games target marker symbols have: Skull = White, Cross = Red, Square = Blue, Triangle = Green, Diamond = Purple, Star = Yellow, Circle = Orange, Moon = Grey
